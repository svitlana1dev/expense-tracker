...
export default function Dashboard() {
  ...
  const handleSend = async (text) => {
    ...
    try {
      ...
      addExpense({ ...parsed, id: Date.now(), description: text, type: 'expense' });
      setMessages((prev) => [
        ...prev.filter((m) => m.id !== THINKING_ID),
        { id: Date.now() + 1, role: 'ai', text: 'Expense added successfully.', expense: parsed },
      ]);
    } catch (err) {
      setMessages((prev) => [
        ...prev.filter((m) => m.id !== THINKING_ID),
    ...
    <div className="flex flex-col h-screen max-w-[820px] mx-auto bg-bg">
      ...
      <header className="flex items-center justify-between px-6 h-[62px] bg-surface border-b border-border shrink-0 gap-4">
        <div className="flex items-center gap-2 shrink-0">
          ...
          <span className="text-base font-bold text-text tracking-tight">Expense Tracker</span>
        </div>

        <div className="flex items-center gap-4">
          {[
            { label: 'Balance',  value: fmt(balance),       cls: balance < 0 ? 'text-expense' : 'text-text' },
            { label: 'Income',   value: fmt(totalIncome),   cls: 'text-income' },
            { label: 'Expenses', value: fmt(totalExpenses), cls: 'text-expense' },
          ].map(({ label, value, cls }, i, arr) => (
            <div key={label} className="flex items-center gap-4">
              <div className="flex flex-col items-end gap-px">
                <span className="text-[10px] font-semibold uppercase tracking-[0.06em] text-muted">{label}</span>
                <span className={`text-sm font-bold tracking-tight ${cls}`}>{value}</span>
              </div>
